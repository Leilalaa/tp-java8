...
public class Lambda_01_Test {
    ...
    public void test_filter_by_age() throws Exception {
    	...
    	PersonPredicate legalPerson = p -> p.getAge()>=18;
    	
        List<Person> personList = Data.buildPersonList(100);

        // TODO result ne doit contenir que des personnes adultes (age >= 18)
        List<Person> result = filter(personList, legalPerson);
    ...
    public void test_filter_by_firstname() throws Exception {
    	...
    	PersonPredicate firstName = p -> p.getFirstname().equals("first_10");

        List<Person> personList = Data.buildPersonList(100);

        // TODO result ne doit contenir que des personnes dont le prénom est "first_10"
        List<Person> result = filter(personList, firstName);
    ...
    public void test_filter_by_password() throws Exception {
        ...
        PersonPredicate checkPassword = p -> p.getAge()>49 && DigestUtils.sha512Hex(p.getPassword()).equals(passwordSha512Hex);
        ...
        List<Person> result = filter(personList, checkPassword);
